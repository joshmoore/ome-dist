...
if not _M_omero.api.__dict__.has_key('_t_StringRStringMap'):
...
if not _M_omero.api.__dict__.has_key('LongPair'):
    ...
    class LongPair(object):
        ...
        def __hash__(self):
            _h = 0
            _h = 5 * _h + __builtin__.hash(self.first)
            _h = 5 * _h + __builtin__.hash(self.second)
            return _h % 0x7fffffff

        def __lt__(self, other):
            if isinstance(other, _M_omero.api.LongPair):
...
if not _M_omero.api.__dict__.has_key('_t_LongPairToStringIntMap'):
